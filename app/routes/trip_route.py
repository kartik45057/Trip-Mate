...
@router.post("/trip", status_code=status.HTTP_201_CREATED, response_model=Trip_Read_without_expenses)
...
def get_filtered_trips_based_on_dates_and_title_for_user(trips_created_by_user_id: Optional[int] = Query(None), start_after: Optional[date] = Query(None), start_before: Optional[date] = Query(None), end_after: Optional[date] = Query(None), end_before: Optional[date] = Query(None), title: Optional[str] = Query(None),  current_user: User_Read = Depends(get_current_user)):
    ...
    try:
        if start_after and start_before and end_after and end_before:
            result = get_user_trips_starting_within_daterange_and_ending_within_daterange(trips_created_by_user_id, start_after, start_before, end_after, end_before)
        elif start_after and start_before and end_after:
            result = get_user_trips_starting_within_daterange_and_ending_after_specified_date(trips_created_by_user_id, start_after, start_before, end_after, title)
        elif start_after and start_before and end_before:
            result = get_user_trips_starting_within_daterange_and_ending_before_specified_date(trips_created_by_user_id, start_after, start_before, end_before, title)
        elif start_after and end_after and end_before:
            result = get_user_trips_ending_within_daterange_and_starting_after_specified_date(trips_created_by_user_id, start_after, end_after, end_before, title)
        elif start_before and end_after and end_before:
            result = get_user_trips_ending_within_daterange_and_starting_before_specified_date(trips_created_by_user_id, start_before, end_after, end_before, title)
        elif start_after and start_before:
            result = get_user_trips_starting_between_specified_dates(trips_created_by_user_id, start_after, start_before, title)
        elif start_after and end_after:
            result = get_user_trips_starting_after_and_ending_after_specified_dates(trips_created_by_user_id, start_after, end_after, title)
        elif start_after and end_before:
            result = get_user_trips_starting_and_ending_within_specified_daterange(trips_created_by_user_id, start_after, end_before, title)
        elif start_before and end_after:
            result = get_user_trips_starting_before_and_ending_after_specified_dates(trips_created_by_user_id, start_before, end_after, title)
        elif start_before and end_before:
            result = get_user_trips_starting_before_and_ending_before_specified_dates(trips_created_by_user_id, start_before, end_before, title)
        elif end_after and end_before:
            result = get_user_trips_ending_between_specified_dates(trips_created_by_user_id, end_after, end_before, title)
        elif start_after:
            result = get_user_trips_starting_after_specified_date(trips_created_by_user_id, start_after, title)
        elif start_before:
            result = get_user_trips_starting_before_specified_date(trips_created_by_user_id, start_before, title)
        elif end_after:
            result = get_user_trips_ending_after_specified_date(trips_created_by_user_id, end_after, title)
        elif end_before:
            result = get_user_trips_ending_before_specified_date(trips_created_by_user_id, end_before, title)
        elif trips_created_by_user_id:
            result = filter_trip_by_user(trips_created_by_user_id, title) 
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An error occurred: {str(e)}")
