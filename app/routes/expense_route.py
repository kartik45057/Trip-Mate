...
from app.database.expense_crud import create_expense_in_db, delete_expense_in_db, get_all_expenses_by_ids, get_expense_details_from_db, update_expense_description_in_db
...
@router.get("/expense", status_code=status.HTTP_200_OK, response_model=Expense_Read)
def get_expense_by_ids(expense_ids: int, current_user: User_Read = Depends(get_current_user), session: Session = Depends(get_session)):
    try:
        result = get_all_expenses_by_ids(expense_ids, session)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An error occurred: {str(e)}")
...
def get_expense_by_id(expense_id: int, current_user: User_Read = Depends(get_current_user), session: Session = Depends(get_session)):
    try:
...
def update_expense_description(expense_id: int, description: str = Query(min_length=10, max_length=100), current_user: User_Read = Depends(get_current_user), session: Session = Depends(get_session)):
    try:
...
def delete_expense(expense_id: int, current_user: User_Read = Depends(get_current_user), session: Session = Depends(get_session)):
    try:
